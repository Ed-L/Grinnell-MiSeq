...
#need remove hyphens for statistical groupings and run the code below normally (for no statistical groupings run code with above metadata)
metadata <- read.csv("GrinnellMiseqv2Rnohypens.csv", row.names=1)
sample <- sample_data(metadata)
sample_data(darte_ed_16s)<- sample
...
#Need to add sample Types
longRareDataTrimID <- longRareData
longRareDataTrimID$variable <- gsub("X0.03.", "",longRareData$variable)
head(longRareDataTrimID)

NeedTypes <- sample_data(darte_ed_16s)
NeedTypes <- cbind(rownames(NeedTypes), NeedTypes)
rownames(NeedTypes) <- NULL
colnames(NeedTypes) <- c("variable","Type","Date","Date_Type")
longRareDatawTypes <- merge(longRareDataTrimID, NeedTypes,by ="variable")

# plotting
options(scipen = 100000)
(rarefaction <- ggplot(data = longRareDatawTypes,
       aes(x = numSampled, y = value, group=variable)) +
  geom_line(aes(color=Date_Type)) +
  labs(x = "Number of Samples", y = "Number of OTUs", color="Sample") +
  theme_bw())
...
darte_ed_16s_filter_re_g = tax_glom(darte_ed_16s_filter_re, "Rank5")
...
a <- subset(dat, dat$Rank5 == "Spartobacteria_unclassified")
b <- subset(dat, dat$Rank5 == "Gp6_unclassified")
c <- subset(dat, dat$Rank5 == "Gp4_unclassified")
d <- subset(dat, dat$Rank5 == "Bacteria_unclassified")
e <- subset(dat, dat$Rank5 == "Gp16_unclassified")
f <- subset(dat, dat$Rank5 == "Chitinophagaceae")
g <- subset(dat, dat$Rank5 == "Betaproteobacteria_unclassified")
h <- subset(dat, dat$Rank5 == "Subdivision3_unclassified")
i <- subset(dat, dat$Rank5 == "Planctomycetaceae")
j <- subset(dat, dat$Rank5 == "candidate_division_WPS-1_unclassified")


summary(aov(Abundance~Date_Type, b))
TukeyHSD(aov(Abundance~Date_Type, Acido))
capture.output(TukeyHSD(aov(Abundance~Date_Type, Acido)),file="Reltukey.csv")

#statistical groupings (from https://www.r-graph-gallery.com/84-tukey-test/)
library(multcompView)

TUKEY <- TukeyHSD(aov(Abundance~Date_Type, Acido))
...
LABELS=generate_label_df(TUKEY,"Date_Type")
...
a=boxplot(Acido$Abundance~Acido$Date_Type , col=my_colors[as.numeric(LABELS[,1])] , ylab="value" , main="", range=6)

# I want to write the letter over each box. Over is how high I want to write it.
over=0.01*max( a$stats[nrow(a$stats),] )

#Add the labels
text( c(1:nlevels(Acido$Date_Type)) , a$stats[nrow(a$stats),]+over , LABELS[,1]  , col=my_colors[as.numeric(LABELS[,1])] )



dat$Date_Type <- factor(dat$Date_Type, levels = c("manure","fall pre-manure soil","manure line","fall post-manure soil","spring time 1 soil","spring time 2 soil"))
#boxplot
...
ggsave(boxplotbyphylum, filename = "BoxPlotbyFamily.pdf", units = "in", width = 10, height = 8, dpi=600)
...
data.sm$Date <- as.factor(sample_data(rare)$Date_Type)
...
#sigtab0 = sigtab0[c("log2FoldChange","padj","Rank2","Rank3","Rank4","Rank5","Rank6")]
...
rbind <- rbind(sigtabL, sigtab1, sigtab2, sigtMmab3)

Mtrack<- subset(rbind, rbind$OTU %in% sigtab0$OTU)

MMtrack$Date_Type <- factor(Mtrack$Date_Type, levels = c("manure line","fall post-manure soil","spring 1 soil","spring 2 soil"))
...
#####
#For supplemental table (looks similiar but need to go to rank 6)
mtrackmanure <- subset(mtrackrelabu.psmelt, Type == "Manure")
mtrackmanure.sum <- ddply(mtrackmanure,.(Date_Type, Type, OTU, Rank2, Rank3, Rank4, Rank5, Rank6), summarise, total = sum(Abundance)/10)

mtracksoil <- subset(mtrackrelabu.psmelt, Type != "Manure")
#need to average but each time point has different sample total due to samples thrown out at rarefraction
mtrackpre <- subset(mtracksoil, Date_Type == "fall pre-manure soil")
mtrackpre.ave <- ddply(mtrackpre,.(Date_Type, Type, OTU, Rank2, Rank3, Rank4, Rank5, Rank6), summarise, total = sum(Abundance)/23)

mtrackpost <- subset(mtracksoil, Date_Type == "fall post-manure soil")
mtrackpost.ave <- ddply(mtrackpost,.(Date_Type, Type, OTU, Rank2, Rank3, Rank4, Rank5, Rank6), summarise, total = sum(Abundance)/23)

mtrackline <- subset(mtracksoil, Date_Type == "manure line")
mtrackline.ave <- ddply(mtrackline,.(Date_Type, Type, OTU, Rank2, Rank3, Rank4, Rank5, Rank6), summarise, total = sum(Abundance)/24)

mtracks1 <- subset(mtracksoil, Date_Type == "spring 1 soil")
mtracks1.ave <- ddply(mtracks1,.(Date_Type, OTU, Type, Rank2, Rank3, Rank4, Rank5, Rank6), summarise, total = sum(Abundance)/22)

mtracks2 <- subset(mtracksoil, Date_Type == "spring 2 soil")
mtracks2.ave <- ddply(mtracks2,.(Date_Type, OTU, Type, Rank2, Rank3, Rank4, Rank5, Rank6), summarise, total = sum(Abundance)/24)
...
mtrackmanure.sum$Type2 = "manure"
mtrackall.ave <- rbind(mtrackmanure.sum, mtrackallsoil.ave)
write.csv(mtrackall.ave, file = "OTUtablesupplemental.csv")
#####
#figure
