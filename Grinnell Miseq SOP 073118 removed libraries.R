...
sigtab0 = sigtab0[c("log2FoldChange","padj","Rank2","Rank3")]
...
sigtabL = sigtabL[c("log2FoldChange","padj","Rank2","Rank3")]
...
sigtab1 = sigtab1[c("log2FoldChange","padj","Rank2","Rank3")]
...
sigtab2 = sigtab2[c("log2FoldChange","padj","Rank2","Rank3")]
...
sigtab3 = sigtab3[c("log2FoldChange","padj","Rank2", "Rank3")]
sigtab3$Date_Type<-"spring 2 soil"

sigtab0 <- setNames(cbind(rownames(sigtab0), sigtab0, row.names = NULL), 
                    c("OTU","log2FoldChange","pval","Phyla","Class","Date_Type"))
...
(Mantrack <- (ggplot(Mtrack, aes(x=Date_Type, y=log2FoldChange, fill=Phyla))+
  ...
  xlab("Sample")))
ggsave(plot = last_plot(), filename = "ManureDispersionPlot.eps", units = "in", width = 6.8, height = 6)
...
Mtrack1 = Mtrack[which(Mtrack$Date_Type == "11/15/16 Soil" & Mtrack$pval < 0.05),]
